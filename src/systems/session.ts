...
/**
 * 1·2·3착 몫. **총합 0.94 는 유지하고 1착만 깎았다** — 0.60 → 0.45 (HT-010).
 *
 * 사용자 판정 — "스탯이 강화된 내 말이 마지막 라운드에 근접하면 거의 필승으로 이겨".
 * 이길 것이 거의 확실한데 판돈의 60% 가 떨어지면 그건 보상이 아니라 **자동 수입**이다.
 *
 * **승률을 안 건드린 것은 의도다.** 상금 배분은 이기는 확률이 아니라 이겼을 때의 값을
 * 정한다 — 사용자가 요청한 것이 그쪽이고, 승률을 만지는 것은 규칙 변경이라 별개다.
 * 깎은 몫은 2·3착으로 옮겼다. 총 수입이 유지되므로 **경제 규모는 그대로**이고,
 * 대신 "이기느냐"보다 **"3착 안에 드느냐"** 가 더 값을 한다.
 */
export const PURSE_SHARE = [0.45, 0.30, 0.19] as const;
