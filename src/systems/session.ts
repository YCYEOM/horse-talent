...
/**
 * 한 판의 경주 수. **12회 고정** (사용자 지정, 2026-08-06).
 *
 * 원래는 8~12 랜덤이었고 이유가 있었다 — 언제 끝날지 모르면
 * "마지막 경주에 전 재산" 이 안 통한다. **고정하면 그 방어가 사라진다** —
 * 12R 이 대상경주(3배)라는 것을 아니까 아껴뒀다 몰아넣는 것이 정답에 가까워진다.
 * 사용자가 실제 플레이(한 판 10분)에서 고정을 택했으므로 그대로 간다.
 * 몰빵 제동은 패리뮤추얼(크게 걸면 내 배당이 내려간다)이 일부 대신한다.
 */
export const RACES_MIN = 12;
export const RACES_MAX = 12;
